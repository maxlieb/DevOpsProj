...
**DadJokes API** is our (Max Liberman & Nati Minz) final project for the Bar Ilan DevSecOps Class 19 course.

It is demonstrating API handling, state management, and integration with DevOps tools like Docker, Kubernetes, and Terraform.

The core of the project is a Python Flask application that fetches dad jokes from external sources *(Originally reddit, but was adjusted later due to limitations of datacenter IPs)* or accepts custom jokes. 
We mostly reused and slightly modified this core from the mid-project.

> *For demo purposes, and to avoid the overhead of setting up a full database, we used the push notification service [ntfy.sh](http://ntfy.sh) as a lightweight data store. 
The data is stored in JSON format and can be accessed via HTTPS requests from any instance, without the need to implement locking mechanisms or manage concurrency.*
> 

## Base app features

- Fetch external jokes and store them automatically in an [**ntfy topic**](https://docs.ntfy.sh/)
...
## Standalone testing - Quick Start (Docker)
...
curl [http://dadjokes-service](http://dadjokes-service/)
or
...
- **Security:** as jokes are usually not information we need to protect, 
This project does not implement much data protection.
    - The information is stored unencrypted on the public [ntfy.sh](http://ntfy.sh) servers, and is publicly accessible - if the “topic” is known.
    - There is support for self-hosted NTFY servers and access keys - hence the NTFY_AUTH env variable.
- Joke list is **auto-pruned** to **30 items** on every save to keep messages small.
- This is not built for long-term storage.

# EKS Cluster via Terraform

This project automatically provisions an **AWS EKS cluster** along with a minimal **VPC setup** using **Terraform**. 
It is designed for learning and demo purposes, providing a ready-to-use Kubernetes environment with essential add-ons and a managed node group.
